...
from src.db_paths import default_core_database_path, resolve_core_database_path
...
class Config:
    ...
    # === 数据库配置 ===
    database_path: str = str(default_core_database_path())
        ...
        return cls(
            ...
            database_path=str(
                resolve_core_database_path(
                    os.getenv('DATABASE_PATH', str(default_core_database_path()))
                )
            ),
    ...
    def get_db_url(self) -> str:
        ...
        自动创建数据库目录（如果不存在）
        """
        db_path = resolve_core_database_path(self.database_path)
