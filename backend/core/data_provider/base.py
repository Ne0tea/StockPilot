...
class DataFetcherManager:
    ...
    def _get_tickflow_fetcher(self):
        """Lazily create a TickFlow fetcher for settings-backed explicit fallbacks."""
    ...
    def _inject_tickflow_daily_fallback(
        self,
        fetchers: List[BaseFetcher],
        market: str,
    ) -> List[BaseFetcher]:
        """
        Insert TickFlow daily-data fallback before Yfinance when configured.

        Ordering goal:
        - Keep existing primary data sources untouched.
        - Add TickFlow only as a fallback supplement.
        - Prefer TickFlow ahead of Yfinance for daily K-lines.
        """
        if market not in {"cn", "hk", "us"}:
            return fetchers
        ...
        if tickflow_fetcher is None:
            return fetchers
        if not self._is_fetcher_available(tickflow_fetcher, capability="daily_data"):
            return fetchers

        result = list(fetchers)
        if any(fetcher is tickflow_fetcher or fetcher.name == "TickFlowFetcher" for fetcher in result):
            return result

        insert_at = len(result)
        for index, fetcher in enumerate(result):
            if fetcher.name == "YfinanceFetcher":
                insert_at = index
                break
        result.insert(insert_at, tickflow_fetcher)
        return result
    ...
    def get_daily_data(
        ...
        if is_hk:
            ...
            fetchers = self._filter_daily_fetchers_for_market(fetchers, "cn")
        fetchers = self._inject_tickflow_daily_fallback(
            fetchers,
            "us" if is_us else "hk" if is_hk else "cn",
        )
        ...
        if is_us:
            ...
            if is_us_index:
                ...
                source_order = ["FinnhubFetcher", "TickFlowFetcher", "YfinanceFetcher"]
            elif prefer_lb:
                source_order = ["LongbridgeFetcher", "FinnhubFetcher", "AlphaVantageFetcher", "TickFlowFetcher", "YfinanceFetcher"]
            else:
                source_order = ["FinnhubFetcher", "AlphaVantageFetcher", "TickFlowFetcher", "YfinanceFetcher", "LongbridgeFetcher"]
